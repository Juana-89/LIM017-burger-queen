...
function Inputs(props)  {
  const { handleChangeName, newName, handleChangeTable, newTable } = props;
    ...
    return (
        ...
        <input type='number' id="input_table" onChange={(e)=>{handleChangeTable(e.target.value)}} value={newTable} className='form-control input_name_customer' required placeholder='0'></input>
        </div>
      )
}
